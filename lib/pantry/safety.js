...
export function clasificarItem(item, restricciones) {
  ...
  if (esVerificado(item)) {
    // RESIDUAL (Slowking H1): un verificado SIN ninguna señal de alérgeno (ni tags OFF ni lista de
    // ingredientes) NO es confirmable. Con una alergia de grupo ANAFILÁCTICO activa aplicamos el
    // MISMO fail-safe que a los no-verificados → DESCONOCIDO (se excluye). Para grupos advertibles
    // (lactosa) NO se fuerza, para no sobre-excluir medio catálogo verificado.
    const sinSenal = !(Array.isArray(item.allergens) && item.allergens.length) && !(Array.isArray(item.ingredientes) && item.ingredientes.length);
    if (sinSenal && grupos.size > 0 && !todosAdvertibles(grupos)) return { status: 'DESCONOCIDO' };
    return { status: 'SEGURO' }; // verificado, tags confiables y sin golpe de nombre
  }
