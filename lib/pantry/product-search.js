...
import { fetchOFF as _fetchOFF, cacheOFF as _cacheOFF } from './off.js';
  ...
  if (nombre) {
    ...
    if (cands.length) {
      ...
      if (decision.modo === 'auto') {
        return { match: 'auto', producto: decision.producto, confianza: decision.confianza ?? null, source: 'db' };
      }
      if (decision.modo === 'disambiguation') {
        return { match: 'disambiguation', candidatos: decision.candidatos || ranked, source: 'db' };
      }
      // 'no_encontrado' local → continúa a fuentes externas (solo si hay barcode)
    }
  }
