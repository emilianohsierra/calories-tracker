...
import { sellosNOM051, inferirForma, esJugo } from './nom051.js';
...
const PROD_SELECT = `id, name, off_id, image_url, presentacion, default_unit, confidence_score, is_user_created, nutri_score, nova_group, country, data_quality, data_quality_score, brands(name), categories(name), product_nutrition(${NUT_COLS}), product_ingredients(ingredient, position), product_additives(additive_code, name)`;
...
export function toProducto(p, brain) {
  ...
  // Fase 5: ingredientes (ordenados por position), aditivos (código E, sin riesgo), sellos NOM-051.
  out.ingredientes = (Array.isArray(p.product_ingredients) ? p.product_ingredients : [])
    .slice()
    .sort((a, b) => (a.position ?? 999) - (b.position ?? 999))
    .map((r) => r.ingredient)
    .filter(Boolean);
  out.aditivos = (Array.isArray(p.product_additives) ? p.product_additives : [])
    .map((r) => ({ additive_code: r.additive_code, name: r.name || null }));
  // Sellos: gate de añadidos por la lista de ingredientes ya cargada (H1). Sin lista → los 3 sellos
  // gated se suprimen; calorías no lleva gate.
  const señales1 = { serving_unit: out.nutricion?.serving_unit, unidad: out.unidad, categoria: out.categoria };
  out.sellos = sellosNOM051(out.nutricion, { forma: inferirForma(señales1), esJugo: esJugo(señales1), ingredientes: out.ingredientes });
  return out;
}
...
function offAProducto(off, codigo = '') {
  ...
  out.data_quality = dq.level;
  out.data_quality_score = dq.score;
  // Fase 5: ingredientes/aditivos del map OFF; sellos computados al vuelo.
  out.ingredientes = Array.isArray(off.ingredientes) ? off.ingredientes.map((it) => it.ingredient).filter(Boolean) : [];
  out.aditivos = Array.isArray(off.aditivos) ? off.aditivos.map((a) => ({ additive_code: a.additive_code, name: a.name || null })) : [];
  const señales2 = { serving_unit: off.serving_unit, unidad: out.unidad, categoria: out.categoria };
  out.sellos = sellosNOM051(out.nutricion, { forma: inferirForma(señales2), esJugo: esJugo(señales2), ingredientes: out.ingredientes });
  return out;
}
