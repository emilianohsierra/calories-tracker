...
// Producto propio NUEVO (sin product_id, capturado por el usuario) → primero alimenta el
// CATÁLOGO vía POST /api/pantry/products; luego se agrega el ITEM a la despensa (/api/pantry).
// Los productos de catálogo (match auto/disambiguation) ya traen product_id → van directo a /pantry.
async function createCatalogProduct(item) {
  try {
    ...
    // sin backend → seguimos sin product_id (fallback local)
  }
  return null;
}
