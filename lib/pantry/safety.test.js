...
describe('VERIFICADO — filtra por allergens ESTRUCTURADOS', () => {
  ...
  it('RESIDUAL (H1): verificado con CERO señal (allergens=[] y sin ingredientes) → fail-safe por grupo', () => {
    // CERO señal real = allergens=[] Y ingredientes=[] (como lo produce readItemsParaMatching sin
    // product_ingredients). anafiláctico → DESCONOCIDO (se excluirá); advertible (lactosa) → SEGURO.
    const sinSenal = { ...mkVer('Producto empacado', []), ingredientes: [] };
    expect(clasificarItem(sinSenal, ['cacahuate']).status).toBe('DESCONOCIDO');
    expect(clasificarItem(sinSenal, ['lacteos']).status).toBe('SEGURO'); // advertible → no sobre-excluye
    // con señal (ingredientes reales) el belt decide; cacahuate oculto → INSEGURO
    expect(clasificarItem({ ...mkVer('Barra energética', []), ingredientes: ['avena', 'cacahuate'] }, ['cacahuate']).status).toBe('INSEGURO');
  });
});
