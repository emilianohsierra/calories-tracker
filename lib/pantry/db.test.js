...
import { norm, cantidadValida, httpUrl } from './text.js';
...
describe('pantry/db · toClientItem (fila BD → shape del cliente)', () => {
  ...
  it('rellena defaults seguros (confianza inválida → user; nulls)', () => {
    ...
    expect(it.image_url).toBe('');
  });

  it('image_url: URL http (OFF) pasa tal cual; PATH de storage queda vacío (lo firma readItems)', () => {
    const off = toClientItem({ id: 'o', nombre: 'Atún', cantidad: 1, imagen: 'https://off/img.jpg' });
    expect(off.imagen).toBe('https://off/img.jpg');
    expect(off.image_url).toBe('https://off/img.jpg');
    const foto = toClientItem({ id: 'f', nombre: 'Yogurt', cantidad: 1, imagen: 'uid-123/pantry/abc.jpg' });
    expect(foto.imagen).toBe('uid-123/pantry/abc.jpg');
    expect(foto.image_url).toBe(''); // path de storage → sin firmar aún
  });
});
...
describe('pantry/text', () => {
  ...
  it('httpUrl acepta SOLO http/https (bloquea data:/javascript:/rutas)', () => {
    expect(httpUrl('https://x/i.jpg')).toBe('https://x/i.jpg');
    expect(httpUrl('  http://x/i.png  ')).toBe('http://x/i.png');
    expect(httpUrl('data:image/png;base64,AAA')).toBe('');
    expect(httpUrl('javascript:alert(1)')).toBe('');
    expect(httpUrl('/local/path.jpg')).toBe('');
    expect(httpUrl(null)).toBe('');
  });
});
