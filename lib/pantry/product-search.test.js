...
describe('ProductSearchService · buscarProducto (cascada cache-first)', () => {
  ...
  it('2d) userCountry prioriza MX: candidatos MX se muestran antes (boostPais)', async () => {
    const cands = [
      { product_id: 'us', nombre: 'Mayo US', country: 'US', confidence: 0.5 },
      { product_id: 'mx', nombre: 'Mayo MX', country: 'MX', confidence: 0.5 },
    ];
    const deps = baseDeps({
      brain: { rankCandidates: (_q, c) => c, decideMatch: (ranked) => ({ modo: 'disambiguation', candidatos: ranked }) },
      deps: { localFuzzy: vi.fn(async () => cands) },
    });
    const r = await buscarProducto(deps, { nombre: 'mayonesa', userId: 'u1' }); // userCountry default MX
    expect(r.match).toBe('disambiguation');
    expect(r.candidatos[0].product_id).toBe('mx'); // MX primero por el boost, aunque venía 2º
  });
...
describe('ProductSearchService · CABLEADO de sellos NOM-051 (H3: gate por ingredientes)', () => {
  const pickFirst = fakeBrain({ pickBestSource: (r) => r[0] });
  it('refresco con jarabe añadido → toProducto expone .sellos con AZÚCARES', () => {
    const p = {
      id: 'r', name: 'Refresco', categories: { name: 'Refrescos' }, default_unit: 'ml',
      product_nutrition: [{ base_unit: 'ml', calories: 80, protein_g: 0, carbs_g: 20, fat_g: 0, azucar_g: 18, nivel: 'verificado' }],
      product_ingredients: [{ ingredient: 'Jarabe de maíz de alta fructosa', position: 1 }],
    };
    const out = toProducto(p, pickFirst);
    expect(out.ingredientes).toEqual(['Jarabe de maíz de alta fructosa']);
    expect(out.sellos.activos).toEqual(expect.arrayContaining(['calorias', 'azucares']));
  });
  it('leche pura → toProducto NO expone sellos de azúcar/grasa (gate)', () => {
    const m = {
      id: 'm', name: 'Leche', categories: { name: 'Leche entera' }, default_unit: 'ml',
      product_nutrition: [{ base_unit: 'ml', calories: 61, protein_g: 3, carbs_g: 5, fat_g: 3, azucar_g: 4.8, saturated_fat_g: 1.9, nivel: 'verificado' }],
      product_ingredients: [{ ingredient: 'Leche entera', position: 1 }],
    };
    const out = toProducto(m, pickFirst);
    expect(out.sellos.activos).not.toContain('azucares');
    expect(out.sellos.activos).not.toContain('grasas_saturadas');
  });
  it('sin ingredientes → sellos gated suprimidos; solo calorías si supera', () => {
    const p = {
      id: 's', name: 'Snack', categories: { name: 'Snacks' }, default_unit: 'g',
      product_nutrition: [{ base_unit: 'g', calories: 500, protein_g: 5, carbs_g: 60, fat_g: 25, azucar_g: 30, saturated_fat_g: 12, sodium_mg: 600, nivel: 'verificado' }],
      product_ingredients: [],
    };
    const out = toProducto(p, pickFirst);
    expect(out.sellos.activos).toEqual(['calorias']);
    expect(out.sellos.azucares).toBe('indeterminado');
  });
});
