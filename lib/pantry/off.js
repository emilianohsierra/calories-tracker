...
export async function cacheOFF(supabase, userId, code, off) {
  ...
  // Item 9: la nutrición es el dato central del cache → si falla, al menos deja rastro (code/details),
  // no la tragues en silencio. No aborta el resto (best-effort), pero ya es diagnosticable.
  const { error: nErr } = await supabase.from('product_nutrition').insert({
  ...
  if (nErr) console.error('[cacheOFF] product_nutrition insert falló:', { code: nErr.code, details: nErr.details, message: nErr.message, product_id: prod.id });
  const { error: bErr } = await supabase.from('barcodes').insert({ product_id: prod.id, code, tipo: 'ean13', source: 'open_food_facts' });
  if (bErr) console.error('[cacheOFF] barcodes insert falló:', { code: bErr.code, details: bErr.details, product_id: prod.id });
  const { error: sErr } = await supabase.from('product_sources').insert({ product_id: prod.id, source_type: 'open_food_facts', external_id: code, url: `https://world.openfoodfacts.org/product/${code}` });
  if (sErr) console.error('[cacheOFF] product_sources insert falló:', { code: sErr.code, details: sErr.details, product_id: prod.id });
  ...
  if (ingr.length) {
    const { error: iErr } = await supabase.from('product_ingredients').insert(
    ...
    if (iErr) console.error('[cacheOFF] product_ingredients insert falló:', { code: iErr.code, details: iErr.details, product_id: prod.id });
  }
  const adit = Array.isArray(off.aditivos) ? off.aditivos : [];
  if (adit.length) {
    const { error: aErr } = await supabase.from('product_additives').insert(
      adit.map((a) => ({ product_id: prod.id, additive_code: a.additive_code, name: a.name ?? null, source: 'open_food_facts', created_by: userId }))
    );
    if (aErr) console.error('[cacheOFF] product_additives insert falló:', { code: aErr.code, details: aErr.details, product_id: prod.id });
  }
  return prod.id;
}
