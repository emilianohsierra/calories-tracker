...
const OFF_URL = (code) =>
  `https://world.openfoodfacts.org/api/v2/product/${encodeURIComponent(code)}.json?fields=product_name,brands,image_url,categories,nutriments,allergens_tags,allergens,nutriscore_grade,nova_group,serving_quantity,countries_tags`;
...
function mapOFF(p) {
  ...
  return {
    ...
    serving_size: n(p.serving_quantity),          // gramos por porción (si OFF lo trae)
    serving_unit: n(p.serving_quantity) != null ? 'g' : null,
    nutricion: {
      base: 'por_100g',
      kcal,
...
function mapSaLHit(h) {
  ...
  return {
    ...
    serving_unit: n(h.serving_quantity) != null ? 'g' : null,
    nutricion: {
      base: 'por_100g',
      kcal,
      prot: n(nut.proteins_100g),
...
export async function searchOFFByName(nombre, { limit = 8 } = {}) {
  ...
  const fields = 'code,product_name,brands,image_url,image_front_url,nutriments,categories_tags,allergens_tags,nutriscore_grade,nova_group,serving_quantity,countries_tags';
