...
export async function addProductImage(supabase, { product_id, image_url, source = 'user', image_type = 'front', is_primary = false, userId = null }) {
  ...
  if (error) console.error('addProductImage:', error.message, error.code || '', error.details || ''); // S-I2: no tragar
  return data?.id || null;
}
...
export async function crearProductoUsuario(supabase, userId, input = {}) {
  ...
  // S-I2 (Slowking): capturar el error de estos inserts — antes se tragaban y devolvían éxito con
  // nutrición faltante. Si la nutrición falla tras crear el producto, se registra con code/details
  // (el producto queda como incompleto; la calidad/reco lo tratan así). No hay éxito silencioso.
  const { error: nutErr } = await supabase
    .from('product_nutrition')
    .insert(nutricionUsuarioRow(productId, input.nutricion, userId, new Date().toISOString()));
  if (nutErr) console.error('crearProductoUsuario nutrition insert:', nutErr.message, nutErr.code || '', nutErr.details || '');
  ...
  return productId; // el producto existe; si nutErr, quedó como incompleto (logueado, no silencioso)
}
