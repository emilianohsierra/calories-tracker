...
import { toClientListItem, readListItems, escribirLista, filaListaItem } from './shopping.js';

// Restricciones REALES del esquema shopping_list_items (supabase/despensa.sql) que el insert debe
// cumplir. Este bloque es el que HABRÍA cazado el bug (null en NOT NULL) que los mocks no cachan.
describe('shopping · filaListaItem cumple el ESQUEMA real (NOT NULL / check / enum)', () => {
  const NOT_NULL = ['list_id', 'user_id', 'cantidad', 'unidad', 'marcado', 'origen'];
  const ORIGEN_ENUM = ['coach', 'manual', 'receta', 'despensa'];

  it('item sin cantidad ni unidad → usa defaults (cantidad=1, unidad="pieza"), NUNCA null', () => {
    const row = filaListaItem({ texto: 'leche' }, { listId: 'L1', userId: 'u1' }); // el caso "anota leche"
    expect(row.cantidad).toBe(1); // NOT NULL default (antes iba null → 23502)
    expect(row.unidad).toBe('pieza'); // NOT NULL default (era el bug)
    for (const k of NOT_NULL) expect(row[k]).not.toBeNull(); // ninguna columna NOT NULL en null
    expect(row.user_id).toBe('u1'); // scoped (RLS with-check)
    expect(row.list_id).toBe('L1');
    expect(ORIGEN_ENUM).toContain(row.origen);
    expect(row.product_id || row.texto_libre).toBeTruthy(); // check(product_id OR texto_libre)
  });

  it('respeta cantidad/unidad/origen provistos y valida el enum de origen', () => {
    const row = filaListaItem({ texto: 'huevo', cantidad: 12, unidad: 'pieza', origen: 'despensa' }, { listId: 'L1', userId: 'u1' });
    expect(row).toMatchObject({ cantidad: 12, unidad: 'pieza', origen: 'despensa' });
    const raro = filaListaItem({ product_id: 'p1', origen: 'inventado' }, { listId: 'L1', userId: 'u1' });
    expect(ORIGEN_ENUM).toContain(raro.origen); // origen fuera del enum → coercionado
    expect(raro.texto_libre).toBe(null);
    expect(raro.product_id).toBe('p1'); // check OK vía product_id
  });
});
