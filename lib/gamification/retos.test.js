...
describe('copySeguro · hardening whitespace (evasión por espacios/tabs/saltos)', () => {
  it('detecta intents con doble espacio, tabs y saltos de línea intermedios', () => {
    for (const bad of [
      'comer  menos',            // doble espacio
      'comer\tmenos',            // tab
      'comer\nmenos',            // salto de línea
      'come   menos   hoy',      // espacios múltiples
      'salta  la  cena',         // multi-palabra con doble espacio
      'quien   come   menos',    // competir por comer menos, espaciado
      '  bajar de  peso  ',      // padding + espacio interno
    ]) {
      expect(copySeguro(bad)).toBe(false);
    }
  });
  it('los 14 copys del catálogo siguen pasando (0 falsos positivos, desayuno≠ayuno)', () => {
    for (const r of RETOS) {
      expect(copySeguro(r.titulo)).toBe(true);
      expect(copySeguro(r.descripcion)).toBe(true);
    }
    expect(copySeguro('Registra tu desayuno')).toBe(true); // desayuno ≠ ayuno
    for (const ok of ['Mantente en tu rango de energía', 'Suma tu proteína', 'ni de más, ni de menos']) {
      expect(copySeguro(ok)).toBe(true);
    }
  });
});
