...
import { registrarComidaFoto, registrarTexto } from './actions.js';

// Stub del cliente Anthropic: la tool estimar_comida devuelve `estimateInput`.
function fakeAnthropic(estimateInput) {
  return {
    messages: {
      create: async () => ({ content: [{ type: 'tool_use', name: 'estimar_comida', input: estimateInput }] }),
    },
  };
}
