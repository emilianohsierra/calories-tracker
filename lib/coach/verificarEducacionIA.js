...
// ¿La cifra kcal aparece como DENSIDAD energética (kcal por gramo)? Entonces es un hecho educativo,
// no una ingesta/meta prescrita. Ej: '4 kcal/g', '9 kcal por gramo', '4 kcal por g'.
function esDensidadKcal(norm, num) {
  const n = String(num).replace(/[.]/g, '\\.');
  return new RegExp(`${n}\\s*kcal\\s*(\\/|por)\\s*g`).test(norm);
}
...
function cifrasSeguras(normIA, base) {
  ...
  // 3) B2: la IA NO puede PRESCRIBIR una cifra nueva que no esté en la base. Una INGESTA/META
  //    calórica nueva es prescripción → bloquea. EXCEPCIÓN: la DENSIDAD energética educativa
  //    (4 kcal/g, 9 kcal por gramo) es un HECHO, no una meta → se permite. Un PESO nuevo grande
  //    (>=2 kg/lb) es meta/ritmo peligroso → bloquea; un peso pequeño (~0.5 kg) SÍ pasa.
  for (const q of paresIA) {
    if (!q.unidad || setBase.has(q.num)) continue;
    if (q.unidad === 'kcal') {
      if (esDensidadKcal(normIA, q.num)) continue; // '4 kcal/g' / '9 kcal por gramo' = densidad, no ingesta
      return { ok: false, motivo: `meta_nueva:${q.num}` };
    }
