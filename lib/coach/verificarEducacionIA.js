...
// Enmascara (borra) las DENSIDADES energéticas PLAUSIBLES ('4 kcal/g', '9 kcal por gramo') para que
// no cuenten como meta/ingesta nueva. POR POSICIÓN (regex global) → borra solo la expresión de
// densidad, no todas las apariciones del valor: '900 kcal/g; apunta a 900 kcal' conserva el '900
// kcal' meta y lo bloquea. PLAUSIBILIDAD: solo valores < 10 kcal/g son densidades reales (carb/prot 4,
// grasa 9, alcohol 7); una "densidad" de 900/1000 kcal/g es gibberish/meta disfrazada → NO se borra.
function enmascararDensidad(norm) {
  return norm.replace(
    /(\d+(?:\.\d+)?)\s*kcal\s*(?:\/|por)\s*g(?:ramo|ramos)?/g,
    (m, num) => (parseFloat(num) < 10 ? ' ' : m),
  );
}
...
function cifrasSeguras(normIA, base) {
  ...
  // 3) B2: la IA NO puede PRESCRIBIR una cifra nueva que no esté en la base. Una INGESTA/META calórica
  //    nueva es prescripción → bloquea. Se enmascara la DENSIDAD energética plausible (4/9 kcal/g,
  //    hecho educativo) ANTES de buscar metas → la densidad no bloquea, pero una meta con el mismo
  //    valor en otra posición SÍ. Un PESO nuevo grande (>=2 kg/lb) → bloquea; peso pequeño (~0.5 kg) pasa.
  const paresMeta = paresNumUnidad(enmascararDensidad(normIA));
  for (const q of paresMeta) {
