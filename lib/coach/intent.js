...
// ¿La persona pide ARMAR/generar su lista del súper (no un item puntual)? → construcción
// determinista desde la despensa (agotados) + faltantes del plan. NUNCA un briefing genérico.
export function intentArmarLista(message) {
  const m = norm(message);
  if (!m) return false;
  const verbo = /\b(arma|armame|hazme|hazme?la|generame|genera|prepara|preparame|crea|creame|planea|planeame|planifica|dame)\b/.test(m);
  const obj = /\b(lista|super|compras|mandado|despensa)\b/.test(m);
  return verbo && obj;
}
