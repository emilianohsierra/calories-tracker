...
describe('verificarEducacionIA · B2 fuga de cifra META + I1 swap de unidad', () => {
  ...
  it('FIX macros: DENSIDAD energética (4 kcal/g, 9 kcal por gramo) sobre base sin cifras → PASA', () => {
    const base = 'Los macronutrientes aportan las calorías y sus funciones. Tu meta ya los reparte.';
    pasa('Proteína y carbohidratos aportan ~4 kcal por gramo y la grasa ~9 kcal por gramo; tu meta los reparte.', base);
    pasa('La proteína da 4 kcal/g y la grasa 9 kcal/g.', base);
  });

  // Hardening Slowking: la exención de densidad es POR-POSICIÓN y solo para valores plausibles (<10).
  describe('densidad kcal/g no puede encubrir una meta (anclas de Slowking)', () => {
    const base = 'Los macronutrientes aportan energía.';
    it('(a) densidad y meta con el MISMO valor → BLOQUEA la meta', () => {
      bloquea('Un gramo da 900 kcal/g; apunta a 900 kcal al dia.', base);
    });
    it('(b) densidad legítima sola → PASA', () => {
      pasa('La proteína aporta 4 kcal por gramo.', base);
    });
    it('(c) meta real sin densidad → BLOQUEA', () => {
      bloquea('Apunta a 1000 kcal al dia.', base);
    });
    it('(d) densidad legítima + meta con OTRO valor → BLOQUEA la meta', () => {
      bloquea('La grasa da 9 kcal/g, baja a 900 kcal.', base);
    });
    it('(e) gibberish: "meta" con /g de valor implausible → BLOQUEA', () => {
      bloquea('Tu meta: 1000 kcal/g.', base);
      bloquea('Apunta a 1000 kcal/g al dia.', base);
    });
  });
});
