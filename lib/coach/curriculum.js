...
export const EXPLICACIONES = {
  ...
  macros: {
    concepto: 'macros',
    titulo: 'Qué son tus macros',
    principiante: 'Tu comida trae tres "macros": proteína (cuida tu músculo y te llena), carbohidratos (te dan energía) y grasas (para tus hormonas). Los tres suman; ninguno es malo.',
    basico: 'Los macronutrientes son proteína, carbohidratos y grasa. La proteína sacia y cuida el músculo (huevo, atún, frijol); los carbohidratos son tu energía (tortilla, arroz, avena); las grasas son hormonales (aguacate, aceite). La idea es equilibrar los tres, no eliminar ninguno.',
    intermedio: 'Proteína (~1.6–2.2 g/kg) preserva masa magra y sacia; los carbohidratos son tu combustible principal, sobre todo peri-entreno (tortilla nixtamalizada, arroz, avena); las grasas (mín ~0.8 g/kg) son hormonales y ayudan a absorber vitaminas liposolubles (aguacate, huevo). En México los tienes fáciles: frijol/huevo/atún para proteína, tortilla/arroz para carbos, aguacate para grasa.',
    avanzado: 'Los macros modulan composición corporal, rendimiento y adherencia. La proteína alta preserva masa magra y aporta saciedad y efecto térmico (TEF); los carbohidratos se ajustan al volumen de entrenamiento (combustible y recuperación de glucógeno); las grasas ≥0.8 g/kg sostienen la función hormonal y la absorción de vitaminas liposolubles. Ningún macro se demoniza: la distribución se adapta a tu objetivo y contexto.',
  },
  calidad_sin_culpa: {
...
export const LECCIONES = {
  ...
  macros: {
    concepto: 'macros',
    titulo: 'Tus macros, sin dramas',
    // ancla a datos reales: metas de macros del motor (nunca inventa). Sin metas → solo la base.
    cuerpo: ({ protObjetivo, carbObjetivo, grasObjetivo } = {}) => {
      const base = 'Tus macros son tres y cada uno suma: la proteína cuida tu músculo y sacia, los carbohidratos son tu energía (sobre todo al entrenar) y las grasas son para tus hormonas. Ninguno es el enemigo.';
      if (protObjetivo > 0 && carbObjetivo > 0 && grasObjetivo > 0) {
        return `${base} Hoy tu meta: ${Math.round(protObjetivo)} g de proteína, ${Math.round(carbObjetivo)} g de carbohidratos y ${Math.round(grasObjetivo)} g de grasa.`;
      }
      return base;
    },
    quiz: {
      pregunta: '¿Qué es cierto sobre los macronutrientes?',
      opciones: [
        'Los tres (proteína, carbos y grasa) cumplen una función y suman',
        'Los carbohidratos son el enemigo, hay que eliminarlos',
        'La grasa engorda, mejor cero grasa',
      ],
      correcta: 0,
      feedback: 'Los tres suman: la proteína cuida tu músculo, los carbohidratos son energía y las grasas son hormonales.',
    },
  },
};
