...
export function parseMessage(content) {
  ...
  // Intento de parseo estructurado si parece nuestro objeto JSON (barato y seguro).
  if (trimmed.startsWith('{')) {
    ...
    // RESCATE: JSON de `responder` TRUNCADO/roto (p.ej. cortado por max_tokens o emitido como texto)
    // → jamás pintamos las llaves crudas al usuario; extraemos al menos el `titular` como texto.
    const m = trimmed.match(/"titular"\s*:\s*"((?:[^"\\]|\\.)*)"/);
    if (m) {
      let titular;
      try { titular = JSON.parse(`"${m[1]}"`); } catch { titular = m[1]; }
      return { kind: 'structured', data: normalizeStructured({ titular }) };
    }
  }
