...
export function seleccionarDelta(mensajes, uptoId, ventana = VENTANA_RECIENTE) {
  ...
  // Marca de agua = índice del último mensaje YA resumido. B2 (#4b): si upto_message_id apunta a un
  // mensaje BORRADO (o que salió de la ventana de lectura acotada #4a), findIndex → -1 = watermark
  // INVÁLIDO. Fallback graceful: se IGNORA y se recomputa desde el inicio del hilo leído (se re-ancla al
  // guardar el nuevo upto → auto-sana en el siguiente turno). Nunca lanza y NO re-persiste el id muerto.
  const uptoValido = uptoId ? arr.some((m) => m.id === uptoId) : false;
  const uptoIdx = uptoValido ? arr.findIndex((m) => m.id === uptoId) : -1;
  const start = uptoIdx + 1;                        // primer mensaje no resumido (sin/ inválido → 0)
  const delta = start < finVentana ? arr.slice(start, finVentana) : [];
  const nuevoUpto = delta.length ? delta[delta.length - 1].id : (uptoValido ? uptoId : null);
