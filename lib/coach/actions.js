...
import { filtrarDespensaSegura, clasificarItem } from '../pantry/safety';
import { quePuedoComer } from '../pantry/suggest';
import { sustituir } from '../pantry/sustituciones';
import { computeTargets } from '../nutrition/compute';
import { COACH_IDS } from '../nutrition/coaches';
import { otorgar as otorgarGam } from '../gamification/otorgar';
import { EVENTOS as EVENTOS_GAM } from '../gamification/eventos';
...
// agregar_a_lista_compras (Fase 7, última tool): PROPONE agregar productos a la lista de compras.
// PROPONE → confirma → aplica (la escritura real es el endpoint CRUD Free). NÚMEROS DEL MOTOR: si la
// persona no da cantidad, se usa el default de restock (1); NUNCA la inventa el modelo. Auto-detecta
// AGOTADOS de la despensa (cantidad<=0) si no se pasan items. SEGURIDAD: nunca propone un alérgeno
// declarado (belt de nombre/ingredientes, igual que el reco). NADA de invención.
const LISTA_CANT_DEFAULT = 1; // cantidad de restock por defecto (constante del motor, no del modelo)

function cantidadMotor(v) {
  const c = Number(v);
  return Number.isFinite(c) && c > 0 ? Math.round(c * 1000) / 1000 : LISTA_CANT_DEFAULT;
}

export async function agregarAListaCompras({ input, ctx }) {
  const p = ctx?.profile || {};
  const restr = [...(p.allergies || []), ...(p.intolerances || []), ...(Array.isArray(p.no_consume) ? p.no_consume : [])];
  const explicit = Array.isArray(input?.items) ? input.items : [];
  let propuesta;
  if (explicit.length) {
    // EXPLÍCITO: el modelo solo trae texto (sin ingredientes) → belt de NOMBRE (lo disponible).
    propuesta = explicit
      .slice(0, 20)
      .map((it) => ({
        texto: String(it?.texto || it?.nombre || '').trim().slice(0, 120),
        product_id: it?.product_id ? String(it.product_id) : null,
        cantidad: cantidadMotor(it?.cantidad), // del usuario si la dio; si no, 1 (motor)
        unidad: it?.unidad ? String(it.unidad).slice(0, 20) : null,
        origen: 'coach',
      }))
      .filter((it) => (it.texto || it.product_id) && findViolations([it.texto], restr).length === 0);
  } else {
    // AUTO: agotados de la despensa (cantidad<=0). SEGURIDAD (Slowking H1): BELT COMPLETO —
    // clasificarItem sobre el ítem REAL (ingredientes/tags/confianza), NO solo el nombre → cierra
    // el alérgeno OCULTO (nombre neutro). Igual que el reco de Fase 6. Restock con cantidad MOTOR (1).
    const items = Array.isArray(ctx?.despensaItems) ? ctx.despensaItems : [];
    propuesta = items
      .filter((i) => Number(i.cantidad) <= 0)
      .filter((i) => clasificarItem(i, restr).status === 'SEGURO')
      .slice(0, 20)
      .map((i) => ({ texto: String(i.nombre || '').slice(0, 120), product_id: null, cantidad: LISTA_CANT_DEFAULT, unidad: i.unidad || null, origen: 'despensa' }))
      .filter((it) => it.texto);
  }
  if (!propuesta.length) return { toolResult: { ok: false, error: 'sin_propuesta' } };

  return {
    listaCompras: propuesta,
    toolResult: {
      ok: true,
      propone: true,
      n: propuesta.length,
      items: propuesta.map((x) => ({ texto: x.texto, cantidad: x.cantidad, unidad: x.unidad, origen: x.origen })),
      nota: 'Propuesta para la lista de compras (cantidades del motor, no inventadas). Se agregan cuando la persona confirme.',
    },
  };
}

// sugerir_sustitucion (Fase 7): dado un producto (que la persona menciona o el PEOR de su despensa
// por sellos de EXCESO / nutri-score) devuelve <=3 alternativas MEJORES y SEGURAS. Todo del MOTOR
// (sustituir() es puro; safety.js filtra cada candidato → NUNCA un alérgeno declarado). El modelo
// SOLO redacta. Grounded: si no hay mejor+segura, lo dice honesto (sin inventar).
const NUTRI_RANK_MEM = { a: 1, b: 2, c: 3, d: 4, e: 5 };
function nutriValMem(s) {
  return NUTRI_RANK_MEM[String(s || '').toLowerCase()] || 0;
}
function sellosLen(i) {
  return Array.isArray(i?.sellos?.activos) ? i.sellos.activos.length : 0;
}
// Peor ítem de la despensa a mejorar: más sellos de EXCESO; a igualdad, peor nutri. Requiere señal.
function peorItem(items) {
  const orden = [...items].sort((a, b) => (sellosLen(b) - sellosLen(a)) || (nutriValMem(b.nutri_score) - nutriValMem(a.nutri_score)));
  const top = orden[0];
  if (!top) return null;
  return sellosLen(top) > 0 || nutriValMem(top.nutri_score) >= 4 ? top : null; // sellos o nutri d/e
}

export function sugerirSustitucion({ input, ctx }) {
  const p = ctx?.profile || {};
  const restr = [...(p.allergies || []), ...(p.intolerances || []), ...(Array.isArray(p.no_consume) ? p.no_consume : [])];
  const items = Array.isArray(ctx?.despensaItems) ? ctx.despensaItems : [];
  const nombreObj = String(input?.producto || '').trim().toLowerCase();

  let target;
  if (nombreObj) {
    target = items.find((i) => String(i.nombre || '').toLowerCase().includes(nombreObj))
      || { nombre: String(input.producto).slice(0, 80), nutri_score: null, sellos: { activos: [] } }; // producto ausente → cualquier candidato seguro sirve
  } else {
    target = peorItem(items);
  }
  if (!target) return { toolResult: { ok: true, sustituciones: [], nota: 'No veo un producto con sellos de exceso que valga la pena cambiar. Dime cuál quieres sustituir y lo reviso.' } };

  // Candidatos = despensa (disponibles). sustituir() pasa CADA uno por safety.js (SOLO SEGURO).
  // Ranking "mejor por OBJETIVO" (Karpathy §B) según la meta del usuario; el objetivo solo REORDENA lo seguro.
  const candidatos = items.map((i) => ({ ...i, disponible: true }));
  const alts = sustituir({ target, candidatos, restricciones: restr, objetivo: p.coach || 'bienestar', opts: { max: 3 } });
  if (!alts.length) {
    return { toolResult: { ok: true, sustituciones: [], objetivo: target.nombre || null, nota: `No encontré una alternativa mejor y segura para ${target.nombre || 'ese producto'} en tu despensa. Si me dices qué buscas, lo reviso.` } };
  }
  return {
    sustituciones: alts,
    toolResult: {
      ok: true,
      n: alts.length,
      objetivo: target.nombre || null,
      sustituciones: alts.map((a) => ({ nombre: a.nombre, razon: a.razon, nutri_score: a.nutri_score, sellos: a.sellos, disponible: a.disponible })),
      nota: 'Alternativas MEJORES y SEGURAS de tu despensa (datos reales; sellos = info de etiqueta, no consejo médico). Los datos son del motor; tú solo redactas.',
    },
  };
}
...
export function esDatoDeSalud(contenido) {
...
export async function actualizarContextoDia({ supabase, userId, input }) {
  ...
  // Gamificación V1 (best-effort, idempotente por día; gated). Check-in del día siempre; entreno solo si 'hecho'.
  await otorgarGam(supabase, EVENTOS_GAM.CHECKIN_COMPLETED, date);
  if (row.entreno_estado === 'hecho') await otorgarGam(supabase, EVENTOS_GAM.WORKOUT_LOGGED, date);
...
export async function registrarComidaFoto({ supabase, userId, input, analysis, ctx }) {
  ...
  const { data: mealRow, error } = await supabase.from('meals').insert({ user_id: userId, ...v.row }).select('id').single();
  if (error) {
    console.error('registrar_comida_foto insert:', error.message);
    return { toolResult: { ok: false, error: 'db' } };
  }
  // Gamificación V1 (best-effort, idempotente por meal_id; gated GAMIFICACION_ON).
  await otorgarGam(supabase, EVENTOS_GAM.MEAL_LOGGED, mealRow?.id);
