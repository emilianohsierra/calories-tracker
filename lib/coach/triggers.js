...
import { NOTIF } from '../gamification/retos'; // V2.1 S4: banco de copy POSITIVO (Karpathy)
...
export const EVENT_TYPES = ['missed_meal', 'low_protein', 'streak', 'weekly_review', 'user_inactivity', 'gamif_hito', 'reto'];
...
export const TOGGLE_COL = {
  ...
  user_inactivity: 'on_user_inactivity',
  gamif_hito: 'on_gamif_hito', // V2.1 S4: hitos de racha + recordatorio de check-in
  reto: 'on_reto',             // V2.1 S4: reto por cerrar
};

// F2 · user_inactivity: días de ausencia en los que avisamos (1 disparo por umbral = back-off
// natural; NO cada día). El re-enganche real es por PUSH (el ausente no abre la app).
export const UMBRALES_INACTIVIDAD = [3, 7, 14];
...
// ── V2.1 S4 · Notificaciones inteligentes de gamificación (copy SOLO del banco positivo de Karpathy;
//    anti-spam/opt-out/dedupe los maneja el cron: seleccionarPorPresupuesto + prefs + UNIQUE + quiet hours).
//    NUNCA presión de racha ansiógena ni 'fallaste'. `seed` = índice del día (variedad determinista). ──
const pickCopy = (arr, seed) => (Array.isArray(arr) && arr.length ? arr[Math.abs(Math.trunc(Number(seed) || 0)) % arr.length] : '');

// hito de racha: a 1 día de un hito (7/14/30) y aún sin registro hoy → "registra hoy y llegas a tu hito".
export function evalHitoRacha({ rachaAyer, registroHoy, seed = 0 } = {}) {
  if (registroHoy) return null;                     // ya registró hoy → lo cubre evalStreak (celebración)
  if (!HITOS_RACHA.includes((Number(rachaAyer) || 0) + 1)) return null; // sólo justo antes de un hito
  return { event_type: 'gamif_hito', slot: 'hito', prioridad: PRIORIDAD.MEDIA, titulo: '¡Estás a un día de tu hito!', cuerpo: pickCopy(NOTIF.racha_dia_gracia, seed) };
}

// reto por cerrar: algún reto activo a 1 de su meta (progreso === meta-1) → empujón suave (positivo).
export function evalRetoCasi({ retos, seed = 0 } = {}) {
  const casi = (retos || []).find((x) => x && !x.completo && Number(x.meta) > 0 && Number(x.progreso) === Number(x.meta) - 1);
  if (!casi) return null;
  return { event_type: 'reto', slot: 'casi', prioridad: PRIORIDAD.MEDIA, titulo: 'Casi cierras tu reto', cuerpo: pickCopy(NOTIF.reto_por_cerrar, seed) };
}

// sin check-in: tarde/noche y aún sin check-in hoy → recordatorio cálido (re-enganche suave). Sin culpa.
export function evalSinCheckin({ checkinHoy, horaLocal, seed = 0 } = {}) {
  if (checkinHoy) return null;
  if (!(r(horaLocal) >= PISO_HORA_RECORDATORIO)) return null; // no por la mañana
  return { event_type: 'gamif_hito', slot: 'checkin', prioridad: PRIORIDAD.BAJA, titulo: '¿Cómo te fue hoy?', cuerpo: pickCopy(NOTIF.reengagement_suave, seed) };
}
...
// ── Trigger 5 · user_inactivity (F2, SOLO push) ──────────────────────────────
// facts: { dias } = días desde el último registro (null si nunca registró → no aplica, es onboarding,
// no churn). Dispara 1 vez en cada umbral (3/7/14) → back-off natural, no todos los días. Tono cálido,
// invitación sin culpa (guardrail: re-enganche, jamás reproche).
export function evalUserInactivity({ dias }) {
  if (dias == null) return null;
  if (!UMBRALES_INACTIVIDAD.includes(r(dias))) return null;
  return {
    event_type: 'user_inactivity',
    prioridad: PRIORIDAD.MEDIA,
    titulo: 'Aquí sigo cuando quieras',
    cuerpo: `Han pasado ${r(dias)} días sin registrar. Cuando quieras retomar, un registro rápido y seguimos juntos.`,
  };
}

// Recomendaciones v2 · B.2 — SWAP POR OBJETIVO (proactivo). Formatea el Nudge a partir de un swap YA
// computado por sustituir() (SEGURO + mejor-por-objetivo; el caller hace el filtro de alérgenos y el
// ranking). PURA. Prioridad BAJA = oferta suave (el gate de presupuesto + quiet hours + back-off deciden
// si se muestra). TCA-safe: POSITIVA sobre la alternativa ("mejor opción"), NUNCA "tu producto es malo".
// facts = { objetivo, target:{nombre}, alternativa:{nombre, razon} }. Sin swap → null (no se ofrece).
export function evalSwapObjetivo({ objetivo, target, alternativa } = {}) {
  if (!objetivo || !target?.nombre || !alternativa?.nombre) return null;
  const razon = alternativa.razon ? ` (${alternativa.razon})` : '';
  return {
    event_type: 'swap_objetivo',
    prioridad: PRIORIDAD.BAJA,
    titulo: 'Una opción que te cuadra mejor',
    cuerpo: `Para tu objetivo, ${alternativa.nombre} encaja mejor que ${target.nombre}${razon}. Tú decides.`,
    meta: { objetivo, target: target.nombre, alternativa: alternativa.nombre },
  };
}

// Días desde el último registro dentro de la ventana de fechas conocidas. null si no hay ninguno.
export function diasDesdeUltimoRegistro(fechas, hoy) {
  const set = fechas instanceof Set ? fechas : new Set(fechas || []);
  if (!set.size) return null;
  let d = hoy;
  let dias = 0;
  // Avanza hacia atrás hasta encontrar el día más reciente con registro.
  for (let i = 0; i <= 400; i += 1) {
    if (set.has(d)) return dias;
    d = diaPrevio(d);
    dias += 1;
  }
  return null;
