...
import {
  ...
  PRIORIDAD, UMBRAL_PROTEINA_G, PISO_HORA_RECORDATORIO,
} from './triggers.js';
...
// ── V2.1 S4 · notificaciones inteligentes de gamificación (copy SOLO positivo; nunca presión de racha) ──
describe('triggers · V2.1 S4 · evalHitoRacha (a 1 día de un hito, sin registro hoy)', () => {
  it('racha ayer = 6 (→ mañana 7) y sin registro hoy → nudge positivo gamif_hito/hito', () => {
    const n = evalHitoRacha({ rachaAyer: 6, registroHoy: false, seed: 0 });
    expect(n).not.toBeNull();
    expect(n.event_type).toBe('gamif_hito');
    expect(n.slot).toBe('hito');
    expect(n.cuerpo).toBeTruthy();
    expect(n.cuerpo.toLowerCase()).not.toContain('fallaste'); // JAMÁS culpa/presión
  });
  it('ya registró hoy → null (lo celebra evalStreak, no re-avisamos)', () => {
    expect(evalHitoRacha({ rachaAyer: 6, registroHoy: true })).toBeNull();
  });
  it('racha ayer no antesala de un hito → null', () => {
    expect(evalHitoRacha({ rachaAyer: 3, registroHoy: false })).toBeNull();
  });
});

describe('triggers · V2.1 S4 · evalRetoCasi (un reto a 1 de su meta)', () => {
  it('reto con progreso === meta-1 → nudge reto/casi', () => {
    const n = evalRetoCasi({ retos: [{ id: 'r', progreso: 2, meta: 3, completo: false }], seed: 1 });
    expect(n).not.toBeNull();
    expect(n.event_type).toBe('reto');
    expect(n.slot).toBe('casi');
    expect(n.cuerpo).toBeTruthy();
  });
  it('reto completo o lejos de la meta → null', () => {
    expect(evalRetoCasi({ retos: [{ progreso: 3, meta: 3, completo: true }] })).toBeNull();
    expect(evalRetoCasi({ retos: [{ progreso: 0, meta: 3, completo: false }] })).toBeNull();
    expect(evalRetoCasi({ retos: [] })).toBeNull();
    expect(evalRetoCasi({})).toBeNull();
  });
});

describe('triggers · V2.1 S4 · evalSinCheckin (tarde/noche sin check-in)', () => {
  it('sin check-in y hora >= piso → recordatorio cálido gamif_hito/checkin', () => {
    const n = evalSinCheckin({ checkinHoy: false, horaLocal: PISO_HORA_RECORDATORIO, seed: 0 });
    expect(n).not.toBeNull();
    expect(n.event_type).toBe('gamif_hito');
    expect(n.slot).toBe('checkin');
    expect(n.prioridad).toBe(PRIORIDAD.BAJA);
  });
  it('ya hizo check-in hoy → null', () => {
    expect(evalSinCheckin({ checkinHoy: true, horaLocal: 20 })).toBeNull();
  });
  it('por la mañana (hora < piso) → null (nunca recordatorio matutino)', () => {
    expect(evalSinCheckin({ checkinHoy: false, horaLocal: 9 })).toBeNull();
  });
});
