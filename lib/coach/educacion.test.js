...
describe('educacion · explicarConcepto (base segura + IA opcional con post-check)', () => {
  ...
  it('reserva no permitida (kill/cap) → base sin llamar al modelo (motivo diagnostico)', async () => {
    ...
    expect(r.via).toBe('determinista');
    expect(r.motivo).toBe('kill');
    expect(redactar).not.toHaveBeenCalled();
  });

  it('reserva con error de RPC → motivo propaga el codigo de Postgres (diagnostico rpc_error)', async () => {
    const r = await explicarConcepto({
      anthropic: {}, reservar: async () => ({ allowed: false, reason: 'rpc_error:42883' }), reembolsar: vi.fn(), redactar: vi.fn(),
    }, { concepto: 'macros', nivel: 'basico', ctx });
    expect(r.via).toBe('determinista');
    expect(r.motivo).toBe('rpc_error:42883');
  });
});
...
describe('educacion · explicarConcepto + JUEZ (2ª etapa, fail-closed)', () => {
  ...
  it('el PRE-FILTRO bloquea ANTES del juez (juez NO se llama, 0 costo extra)', async () => {
    ...
    expect(r.via).toBe('determinista');
    expect(r.motivo).toBe('prefiltro_bloqueo');
    expect(juez).not.toHaveBeenCalled();
    expect(reembolsar).toHaveBeenCalledOnce();
  });

  it('pre-filtro pasa + juez PELIGROSO → fallback + reembolso (motivo juez_peligroso)', async () => {
    const reembolsar = vi.fn();
    const r = await explicarConcepto(okDeps({ juez: async () => ({ peligroso: true, categoria: 'perifrasis' }), reembolsar }),
      { concepto: 'proteina', nivel: 'basico', ctx });
    expect(r.via).toBe('determinista');
    expect(r.motivo).toBe('juez_peligroso');
    expect(reembolsar).toHaveBeenCalledOnce();
  });

  it('pre-filtro pasa + juez SEGURO (peligroso:false) → IA (personalizado=true en la respuesta)', async () => {
    const r = await explicarConcepto(okDeps({ juez: async () => ({ peligroso: false }) }),
      { concepto: 'proteina', nivel: 'basico', ctx });
    expect(r.via).toBe('ia');
    expect(r.via === 'ia').toBe(true); // el route expone personalizado = (via === 'ia')
  });

  it('contrato personalizado: via determinista → personalizado=false', async () => {
    const r = await explicarConcepto({ anthropic: null }, { concepto: 'deficit', nivel: 'basico', ctx });
    expect(r.via).toBe('determinista');
    expect(r.via === 'ia').toBe(false);
  });
