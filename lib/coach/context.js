...
import { bloqueGamificacion } from './gamificacionContext';
...
export async function assembleContext(supabase, userId, conversationId) {
  ...
  const [{ data: profile }, { data: targets }, { data: meals }, { data: history }, { data: summary }, { data: dayState }, { data: memorias }, despensa, despensaItems, gamiBlock] =
    await Promise.all([
      ...
      // S3 (Gamificación V2.1): bloque read-only para el coach. Deploy-safe → '' si v2 off.
      bloqueGamificacion(supabase, userId),
    ]);
  ...
  return {
    ...
    // S3: el bloque de gamificación (V2.1) va en la capa VOLÁTIL (contexto_dia), no en el system
    // cacheado. gamiBlock ya viene deploy-safe ('' si v2 off / tablas ausentes).
    contextoDia: `${contextoDiaBlock(ctx)}${gamiBlock || ''}`,
