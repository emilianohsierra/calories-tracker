...
import {
  ...
  SRS, CATALOGO, ITEMS_REPASO, TEMAS_REPASO, sumarDias,
} from './repaso.js';
import { CONCEPTOS_MVP, leccionDe, explicacionDe } from './curriculum.js';
...
describe('repaso · catálogo canónico (12 ítems, sincronizado con Karpathy)', () => {
  it('exactamente 4 conceptos × 3 formas = 12 ítems', () => {
    expect(ITEMS_REPASO.length).toBe(12);
    expect(TEMAS_REPASO.sort()).toEqual(['calidad_sin_culpa', 'deficit', 'macros', 'proteina']);
    for (const tema of TEMAS_REPASO) expect(CATALOGO[tema].formas.length).toBe(3); // ≥2 para re-enseñar
  });
  it('cada ítem: opciones=3, correcta en rango [0,2] y su opción existe; feedbacks presentes', () => {
    for (const it of ITEMS_REPASO) {
      expect(it.opciones.length).toBe(3);
      expect(it.correcta).toBeGreaterThanOrEqual(0);
      expect(it.correcta).toBeLessThanOrEqual(2);
      expect(it.opciones[it.correcta]).toBeTruthy();
      expect(it.feedback_ok && it.feedback_no).toBeTruthy();
    }
  });
  it('correcta varía (no siempre el mismo índice) y cada concepto tiene formas únicas', () => {
    expect(new Set(ITEMS_REPASO.map((i) => i.correcta)).size).toBeGreaterThan(1);
    for (const tema of TEMAS_REPASO) {
      const ids = CATALOGO[tema].formas.map((f) => f.id);
      expect(new Set(ids).size).toBe(ids.length);
    }
  });
  it('construirItemRepaso rellena el contexto con cifras del motor', () => {
    const forma = CATALOGO.proteina.formas[0]; // v1 usa prot_consumida/prot_meta
  ...
  it('TCA-safe: la OPCIÓN CORRECTA nunca contiene un mito/restricción (es la respuesta a marcar)', () => {
    // Solo se linta la opción CORRECTA: los distractores SON mitos y los feedbacks los NOMBRAN para
    // refutarlos (p.ej. "ningún alimento quema grasa"), así que un lint sobre feedback daría falsos
    // positivos. El contenido completo está validado por Karpathy + QA de Slowking; esDatoDeSalud es el
    // cinturón runtime sobre el ítem ensamblado.
    const PROH = ['come menos', 'saltate', 'sáltate', 'prohibi', 'veneno', 'engorda', 'quema grasa', 'bascula', 'báscula', 'culpa', 'pasar hambre', 'detox', 'eliminar'];
    for (const it of ITEMS_REPASO) {
      const correcta = String(it.opciones[it.correcta]).toLowerCase();
      for (const p of PROH) expect(correcta.includes(p), `${it.concepto}/${it.forma} opción correcta contiene "${p}"`).toBe(false);
    }
  });
});
...
describe('repaso · macros integrado (deja de estar huérfano; #1 lección Karpathy)', () => {
  it('macros ya está en CONCEPTOS_MVP (tiene lección que lo siembra)', () => {
    expect(CONCEPTOS_MVP).toContain('macros');
  });
  it('leccionDe(macros): cuerpo con metas del motor (slots) + quiz válido', () => {
    const lec = leccionDe('macros');
    expect(lec).toBeTruthy();
    const cuerpo = lec.cuerpo({ protObjetivo: 120, carbObjetivo: 200, grasObjetivo: 60 });
    expect(cuerpo).toContain('120'); expect(cuerpo).toContain('200'); expect(cuerpo).toContain('60');
    // sin metas → solo la base (nunca inventa cifras)
    expect(lec.cuerpo({})).not.toMatch(/\d/);
    expect(lec.quiz.opciones.length).toBe(3);
    expect(lec.quiz.opciones[lec.quiz.correcta]).toBeTruthy();
  });
  it('explicacionDe(macros) existe en las 4 profundidades', () => {
    for (const n of ['principiante', 'basico', 'intermedio', 'avanzado']) {
      expect(explicacionDe('macros', n).texto).toBeTruthy();
    }
  });
  it('el quiz de la lección alinea con los mitos del catálogo de repaso macros (carbos-enemigo / grasa-engorda)', () => {
    const distractores = leccionDe('macros').quiz.opciones.filter((_, i) => i !== leccionDe('macros').quiz.correcta).join(' ').toLowerCase();
    expect(distractores).toMatch(/enemigo|elimin/); // mito carbos
    expect(distractores).toMatch(/engorda|cero grasa/); // mito grasa
    // los ítems de repaso macros rechazan esos mismos mitos
    const repMacros = ITEMS_REPASO.filter((i) => i.concepto === 'macros');
    expect(repMacros.length).toBe(3);
  });
});
