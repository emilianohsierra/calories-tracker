...
export async function explicarConcepto(deps, { concepto, nivel, ctx } = {}) {
  ...
  const caer = async () => { await reembolsar(); return { ...base, via: 'determinista' }; };
  try {
    const texto = await redactar(base);
    // (a) Pre-filtro morfológico + protección de cifras.
    if (!verificarEducacionIA(texto, base.texto).ok) return caer();
    // (b) Juez LLM (si está cableado). FAIL-CLOSED: solo pasa con peligroso === false explícito.
    if (juez) {
      let veredicto;
      try { veredicto = await juez(texto); }
      catch { return caer(); } // error/timeout/parse → peligroso por diseño
      if (!veredicto || veredicto.peligroso !== false) return caer();
    }
