...
describe('Sanity checks por coach (plan/ola1-formulas-coaches.md)', () => {
  ...
  it('RECOMPOSICIÓN — hombre 30/80/178, moderado, grasa 22% (déficit ligero −5%)', () => {
    const t = computeTargets({
      coach: 'recomposicion',
      sex: 'male',
      age: 30,
    ...
    expect(t.method).toBe('katch');
    expect(t.kcal_target).toBe(2530);
    expect(t.protein_g).toBe(176); // 2.2 g/kg
    expect(t.fat_g).toBe(72); // 0.9 g/kg
    expect(t.carbs_g).toBe(294);
  });

  it('RECOMPOSICIÓN — borde mujer 42kg/35% cae al piso 1200 (seguridad)', () => {
    const t = computeTargets({
      coach: 'recomposicion',
      sex: 'female',
      age: 30,
    ...
    expect(t.kcal_target).toBe(1200); // piso femenino manda pese al −10%
  });
