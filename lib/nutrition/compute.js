...
// Recomposición: déficit ligero según % grasa (primer umbral cumplido); default 0.05 sin dato.
function deficitRecomp(coach, bodyFatPct, sex) {
  if (bodyFatPct == null) return coach.defaultDeficit;
  const tabla = coach.deficitByBodyFat[sex] || coach.deficitByBodyFat.male;
  for (const row of tabla) {
    if (bodyFatPct >= row.bf) return row.def;
  }
  return 0.0;
}
...
export function computeTargets(profile) {
  ...
  } else if (coach.id === 'recomposicion') {
    // Déficit LIGERO modulado por % grasa (tope duro 10%); proteína alta.
    tdeeVal = tdeeStd;
    const defPct = deficitRecomp(coach, profile.body_fat_pct ?? null, sex);
    objetivo = tdeeVal * (1 - defPct);
    const deficitDia = tdeeVal - objetivo;
    objetivo = aplicaTopesDeficit({ objetivo, tdeeVal, bmrVal, sex, weightKg, deficitDia });
  } else {
